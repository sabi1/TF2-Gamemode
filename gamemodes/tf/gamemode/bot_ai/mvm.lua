...
local cv_allow_carrier_fight = CreateConVar("tf_mvm_bot_allow_flag_carrier_to_fight", "0", {FCVAR_ARCHIVE, FCVAR_NOTIFY})
...
function M:SelectAction(bot, state)
	...
	local forceIgnoreCombat = bot.TF_MVM_IgnoreCombat == true or bot.TF_MVM_IgnoreEnemies == true
	local now = CurTime()
	...
	if IsValid(carrier) then
		...
		local desired = carrier:GetPos()
		if (not carrierInSpawn) and bot:GetPos():Distance(desired) <= cv_escort_range:GetFloat() and IsValid(state.vision.currentThreat) then
			return {
				mode = "mvm_attack_flag_defenders",
				targetEnt = state.vision.currentThreat,
				targetPos = state.vision.currentThreat:GetPos(),
		...
		return {
			...
			targetEnt = nil,
			targetPos = getEscortOffsetPos(bot, desired),
			routeType = "mvm_push",
