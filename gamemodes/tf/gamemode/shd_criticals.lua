...
function GM:ShouldCrit(ent, inf, att, hitgroup, dmginfo) 
	...
	if (att:IsPlayer() and att:HasPlayerState(PLAYERSTATE_CRITBOOST)) then 
	...
	if (att:IsPlayer()) then
		local activeWeapon = att:GetActiveWeapon()
		if IsValid(activeWeapon) and string.find(activeWeapon:GetClass(), "tf_weapon") and activeWeapon.Critical and activeWeapon:Critical(ent,dmginfo) then
			dmginfo:SetDamageType(bit.bor(dmginfo:GetDamageType(),DMG_ACID))
			return true
		end
	end
