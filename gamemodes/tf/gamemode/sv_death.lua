...
function lookForRandomPlayer(bot)
	local npcs = {}
	...
	for k,v in ipairs(player.GetAll()) do
		...
		if (v:GetObserverTarget() == ent) then
			timer.Simple(2.0, function()
				if (v:GetObserverTarget() == ent) then
					v:ConCommand("tf_spectate_respawn2")
				end
			end)
		end
	end
...
function GM:PlayerDeath(ent, inflictor, attacker)
	...
	if (!GAMEMODE.RoundHasWinner) then
		if (GetConVar("mp_disable_respawn_times"):GetBool()) then
			...
			ent.NextSpawnTime = CurTime() + 9999
		end
	else
	...
	timer.Create("Respawn"..ent:EntIndex(), 6.5, 1, function()
		if (!GAMEMODE.RoundHasWinner) then
			...
			if (!GetConVar("mp_disable_respawn_times"):GetBool()) then
				if (ent:Team() != TEAM_RED && ent:Team() != TEAM_BLU) then
					ent:Spawn()
				end
			end
			if (!GetConVar("mp_disable_respawn_times"):GetBool()) then
				if !ent:Alive() and !ent.TFBot then
					...
					ent:SpectateEntity(lookForRandomPlayer(ent))
				end
			end
			if IsValid(ent) and GetConVar("mp_disable_respawn_times"):GetBool() then
				if !ent:Alive() then
					ent:Spawn()
				end
			end
