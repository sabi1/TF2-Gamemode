...
function TF_MVMShop:LoadUpgrades()
    ...
    if SERVER then
        ...
        print("[MVMShop] loaded TF2 base upgrades from " .. tostring(baseData.path) .. extraMsg .. " entries=" .. tostring(#TF_MVMShop.Upgrades))
    end
end
...
if SERVER then
    ...
    local function BuildPanel(payload)
        ...
        local className = string.lower(tostring((IsValid(subjectPly) and subjectPly:GetPlayerClass()) or "scout"))
        ...
        classIcon:SetImage(ResolveClassPortraitIcon(subjectPly, className))
        ...
        local tabSep = vgui.Create("DPanel", card)
        ...
        function list:PopulateRows()
            ...
            for _, up in ipairs(payload.upgrades or {}) do
                ...
                icon:SetSize(math.floor((res.rowIconW or 30) * uiScale), math.floor((res.rowIconH or 30) * uiScale))
        ...
        function frame:ApplyPayload(newPayload)
            ...
            if IsValid(classIcon) then
                ...
                classIcon:SetImage(ResolveClassPortraitIcon(subjectPly, cls))
            end
