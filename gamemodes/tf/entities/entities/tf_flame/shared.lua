...
function ENT:Initialize()
	...
	local activeWeapon = IsValid(owner) and owner:GetActiveWeapon() or nil
	local itemData = IsValid(activeWeapon) and activeWeapon.GetItemData and activeWeapon:GetItemData() or nil
	if itemData and itemData.model_player == "models/workshop/weapons/c_models/c_drg_phlogistinator/c_drg_phlogistinator.mdl" then
		timer.Create("Particle?"..self:EntIndex(), 0.0, 1, function()		
			if self:IsValid(self.WModel2) then	
				ParticleEffectAttach("new_flame", PATTACH_ABSORIGIN_FOLLOW, self, 0)
			else
