...
local function zone_matches_tags(zone, bot)
	...
	for _, tag in ipairs(tags) do
		if tag == "common" then return true end
		if tag == botClass then return true end
		if tag == "bomb_carrier" and bot.HasTheFlag and bot:HasTheFlag() then return true end
		if bot.HasTag then
...
function ENT:Initialize()
	...
	if SERVER then
		GAMEMODE.NavPreferZones = GAMEMODE.NavPreferZones or {}
		GAMEMODE.NavPreferZones[self] = true
	end
end
...
function ENT:OnRemove()
	if SERVER and GAMEMODE.NavPreferZones then
		GAMEMODE.NavPreferZones[self] = nil
	end
end
...
function TF_IsPointInNavPreferZone(bot, pos)
	if not (GAMEMODE and GAMEMODE.NavPreferZones) then return false end
	for zone in pairs(GAMEMODE.NavPreferZones) do
