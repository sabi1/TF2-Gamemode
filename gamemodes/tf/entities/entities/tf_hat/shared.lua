...
hook.Add("EntityEmitSound", "MouthFix", function(snd)
	...
	if (IsValid(snd.Entity)) then
		local soundAwareness = GetConVar("tf_bot_sound_awareness")
		if soundAwareness and soundAwareness:GetBool() then
			local minInterval = math.max(GetConVar("tf_bot_sound_awareness_interval"):GetFloat(), 0.05)
			snd.Entity._nextSoundSense = snd.Entity._nextSoundSense or 0
			if snd.Entity._nextSoundSense <= CurTime() then
				snd.Entity._nextSoundSense = CurTime() + minInterval
				local awarenessRadius = math.max(GetConVar("tf_bot_sound_awareness_radius"):GetFloat(), 128)
				for k, v in ipairs(ents.FindInSphere(snd.Entity:GetPos(), awarenessRadius)) do
					if ((v:IsPlayer() and v.TFBot) and !IsValid(v.TargetEnt) and v:EntIndex() != snd.Entity:EntIndex()) then
						if (snd.Entity.TFBot and v:IsTFPlayer()) then
							...
							if (discoverychance > math.random(1,100) and !snd.Entity.LookAtEntity:IsFlagSet(FL_NOTARGET) and snd.Entity.LookAtEntity:EntityTeam() ~= TEAM_NEUTRAL and !IsValid(snd.Entity.TargetEnt) and snd.Entity.LookAtEntity:GetPos():Distance(v:GetPos()) < 500 and snd.Entity.LookAtEntity:IsTFPlayer() and !snd.Entity.LookAtEntity:IsFriendly(v)) then
						...
						if (discoverychance > math.random(1,100) and !v.LookAtEntity:IsFlagSet(FL_NOTARGET) and v.LookAtEntity:EntityTeam() ~= TEAM_NEUTRAL and !IsValid(v.TargetEnt) and v.LookAtEntity:GetPos():Distance(v:GetPos()) < 500 and v.LookAtEntity:IsTFPlayer() and !v.LookAtEntity:IsFriendly(v)) then
...
if (IsMounted("left4dead") or IsMounted("left4dead2")) then 
		...
		if (GetConVar("tf_bot_sound_awareness"):GetBool() and snd.Entity:IsTFPlayer() and GAMEMODE:EntityTeam(snd.Entity) != TEAM_GREEN) then
		...
		if (GetConVar("tf_bot_sound_awareness"):GetBool() and snd.Entity:IsTFPlayer() and !snd.Entity:IsNextBot()) then
