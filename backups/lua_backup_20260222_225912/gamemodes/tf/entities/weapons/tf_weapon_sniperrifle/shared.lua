...
function SWEP:ZoomOut()
	...
	if (owner:GetVelocity():Length() != 0) and owner.DoAnimationEvent then
		owner:DoAnimationEvent(ACT_MP_ATTACK_STAND_POSTFIRE, true)
	end
	self.NextAutoZoomOut = nil
	if self.ZoomStatus then
...
function SWEP:Holster()
	...
	if SERVER and IsValid(self.Owner) and self.Owner.DoAnimationEvent then
		self.Owner:DoAnimationEvent(ACT_MP_ATTACK_STAND_POSTFIRE, true)
	end
	self:ZoomOut()
