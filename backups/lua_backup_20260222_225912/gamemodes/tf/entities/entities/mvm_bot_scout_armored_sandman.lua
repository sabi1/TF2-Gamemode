...
function ENT:CustomOnInitialize(bot)
	timer.Create("SetModel" .. bot:EntIndex(), 0.1, 10, function()
		...
		if IsValid(wep) and wep.Secondary then
			wep.Secondary.Delay = math.max((wep.Secondary.Delay or 0.5) * 0.05, 0.25)
		end
	end)
end

list.Set("NPC", "mvm_bot_scout_armored_sandman", {
	Name = ENT.PrintName,
	Class = "mvm_bot_scout_armored_sandman",
