...
function ENT:RestoreSandmanThrow(ply)
	...
	for _, wep in ipairs(ply:GetWeapons()) do
		if IsValid(wep) and string.StartWith(wep:GetClass(), "tf_weapon_bat_wood") then
			wep:SetNextSecondaryFire(CurTime())
			local ammo_type = wep.Primary and wep.Primary.Ammo
			if ammo_type and ammo_type ~= "none" then
				ply:SetAmmoCount(1, ammo_type)
			end
			if wep.SetSandmanRechargeEndTime then
				wep:SetSandmanRechargeEndTime(nil)
			end
			restored = true
		end
	end
...
function ENT:TryScoutPickup(ply)
	if self.Dead or not self:CanScoutPickup(ply) then return false end
	if not self:RestoreSandmanThrow(ply) then return false end
	
	ply:EmitSoundEx("AmmoPack.Touch")
	self.Dead = true
...
function ENT:Touch(ent)
	if self:TryScoutPickup(ent) then
		return
	end
...
function ENT:PhysicsCollide(data, physobj)
	if data.HitEntity and self:TryScoutPickup(data.HitEntity) then
		return
	end
