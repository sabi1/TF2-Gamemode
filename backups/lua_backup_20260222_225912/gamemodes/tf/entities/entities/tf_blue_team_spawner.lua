...
function ENT:SetupDataTables()
	self:NetworkVar("Int", 0, "MaxAliveBots", {
		KeyName = "max_alive_bots",
		Edit = {
			title = "Max Alive Bots",
			type = "Int",
			order = 1,
			min = 1,
			max = 128,
...
local function CountAliveBotsOnTeam(teamId)
	local count = 0
	for _, bot in ipairs(player.GetBots()) do
		if IsValid(bot) and bot:Team() == teamId and bot:Alive() then
			count = count + 1
		end
	end
	return count
...
function ENT:Initialize()
	...
	self.spawnsblu = {}
	self.spawnsred = {}	
	if self:GetMaxAliveBots() < 1 then
		self:SetMaxAliveBots(12)
	end
	...
	if (!self.WaveStarted) then
		...
		timer.Create("BotSpawner"..self:EntIndex(), 4, 0, function()
			if SERVER then 
				local slef = self
				local spawn = self
				local maxAlive = math.max(self:GetMaxAliveBots(), 1)
				if (CountAliveBotsOnTeam(self.Team) < maxAlive) then
					local bots = {
						"tf_red_bot",
						"tf_red_bot_soldier",
						"tf_red_bot_pyro",
